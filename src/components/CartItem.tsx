...
export const CartItem: React.FC<CartItemProps> = ({ id, title, size, price, count, imageUrl, classification, structure  }) => {
  ...
  const onClickMinus = () => {
    dispatch(minusItem({ id, size } as CartItemType));
  };
  const onClickRemove = () => {
    dispatch(removeItem({ id, size } as CartItemType));
  };
  return (
